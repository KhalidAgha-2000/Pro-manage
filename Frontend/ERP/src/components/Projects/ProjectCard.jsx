...
const ProjectCard = ({ _id, name, teamName, in_progress, numberOfEmployees, changeProjectStatus }) => {

    return (
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ y: [50, 0], opacity: [0, 0, 1] }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.3 }}
            key={_id} className='mt-10 w-[32%] h-fit max-w-sm relative overflow-hidden bg-dark flex flex-col justify-center items-start  text-sidebar p-4 rounded-xl'>
            ...
            <div className='w-full auto flex flex-col justify-start gap-y-1 mt-1'>
                ...
                <div className='flex'>
                    <input type="checkbox" name="inProgress" id="inProgress" checked={in_progress ? true : false}
                        className='accent-orange w-4 mx-1 hover:scale-150  transition duration-200 ease-in-out cursor-pointer'
                        // Change Project Status
                        onChange={(e) => { changeProjectStatus(e, _id) }}
                    />
                    <p >in_progress</p>
                </div>

                <p><span className='bg-orange text-sidebar px-1 mx-1 rounded-sm'>{numberOfEmployees && numberOfEmployees || '0'}</span>Employee(s)</p>
